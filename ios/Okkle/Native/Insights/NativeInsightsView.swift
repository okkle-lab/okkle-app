  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 24) {
      if let plan = shift.todayPlan, let peak = plan.peakWindow {
        // Prefer today's own weekday zones — falls back to the whole
        // period's when today's slice is too sparse to say anything, so a
        // quiet weekday doesn't lose the recommendation entirely.
        let placeZones = shift.zonesByWeekday[plan.weekday]?.isEmpty == false
          ? shift.zonesByWeekday[plan.weekday]!
          : shift.zones
        NativeAiCard {
          VStack(alignment: .leading, spacing: 14) {
            insightHeader("Where to go", systemImage: "map.fill")
            NativeRecommendationHeatMap(trips: trips, zones: placeZones, timeLabel: peak.label)
            NativeTopAreasList(zones: placeZones, limit: 3, showShareBar: true, showDistance: true)
          }
        }
...
struct NativeWeeklyInsightPanel: View {
  ...
  /// Which day the card reflects: your tap, else today (if you worked it),
  /// else your busiest day.
  ...
  /// That weekday's own zone ranking — falls back to the whole week's when
  /// the day's own slice is too sparse to say anything, so a quiet weekday
  /// doesn't lose the recommendation entirely.
  private var activeZones: [NativeZonePoint] {
    shift.zonesByWeekday[activeWeekday]?.isEmpty == false ? shift.zonesByWeekday[activeWeekday]! : shift.zones
  }

  /// Compact, mostly-numeric chips for the selected day.
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 24) {
      // Card 1 — a day-pill row above one map, same cockpit language as
      // Today. Each pill's own zone ranking is genuinely computed from just
      // that weekday's activity (zonesByWeekday), so tapping a day actually
      // changes *where* to go, not just the time badge.
      NativeAiCard {
        VStack(alignment: .leading, spacing: 16) {
          ...
          NativeTopAreasList(zones: activeZones, limit: 3, showShareBar: true, showDistance: true)
